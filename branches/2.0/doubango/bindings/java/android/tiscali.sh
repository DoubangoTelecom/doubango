...
# wiPhone parameters
export WIPHONE_PARAMS="FFMPEG=no H264=no THEORA=no OPENCORE_AMR=no SPEEX_DSP=yes SPEEX_JB=yes SPEEX_RESAMPLER=yes SPEEX_DENOISE=no SPEEX=no ILBC=no LIBGSM=no G729=no"
export _BT=static
...
do
	...
	do
		...
		make PROJECT=$project BT=$_BT $@ NEON=$IS_NEON $WIPHONE_PARAMS all
	done

	echo building tinyWRAP....
	make PROJECT=tinyWRAP clean
	make PROJECT=tinyWRAP BT=shared LT=$_BT $@ NEON=$IS_NEON $WIPHONE_PARAMS all
done