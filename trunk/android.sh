...
export ANDROID_BIN=/cygdrive/c/android-ndk/build/prebuilt/windows/arm-eabi-4.2.1/bin
...
export CFALGS=$CPPFLAGS

export LDFLAGS="-Wl,--no-gc-sections,--entry=main,-rpath=/system/lib,-rpath-link=$ANDROID_PLATFORM/arch-arm/usr/lib,-dynamic-linker=/system/bin/linker -L$ANDROID_PLATFORM/arch-arm/usr/lib"
export LDFLAGS="$LDFLAGS -Wl,--no-undefined"
export LDFLAGS="$LDFLAGS -nostdlib -lc -L$OUTPUT_DIR"
export LDFLAGS="$LDFLAGS -Wl,-soname,libtinySAK.so,-Bsymbolic,-shared,--whole-archive"